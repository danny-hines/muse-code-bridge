...
if ! "$node_bin" -e 'process.exit(Number(process.versions.node.split(".")[0]) >= 22 ? 0 : 1)'; then
  echo 'Muse Bridge requires Node.js 22 or newer.' >&2
  exit 1
fi
...
const plugin = read('plugins/muse-bridge/.codex-plugin/plugin.json');
if (marketplace.name !== 'muse-bridge' || plugin.name !== 'muse-bridge' ||
    !marketplace.plugins.some(p => p.name === 'muse-bridge' && p.source.path === './plugins/muse-bridge')) {
...
for (const path of ['scripts/server.mjs', 'scripts/launch.sh', 'skills/muse/SKILL.md', '.mcp.json']) {
  accessSync(join(root, 'plugins/muse-bridge', path));
}
console.log(`Package checked: Muse Bridge ${plugin.version}`);
JS
...
printf '%s\n' "$installed_plugins" | "$node_bin" --input-type=module -e '
  ...
  const existing = list.installed.find(p => p.name === "muse-bridge" && p.marketplaceName !== "muse-bridge");
  if (existing) {
    console.error(`Muse Bridge is already installed from ${existing.marketplaceName}.`);
...
"$codex_bin" plugin add muse-bridge@muse-bridge
echo
echo 'Muse Bridge installed. Start a NEW local conversation in ChatGPT desktop.'
echo 'Select Muse Bridge and ask: Ask Muse to review my changes.'
