...
set -eu
hosts='' check=false login=false opencode_version=auto auth='' key_file=''
while [ "$#" -gt 0 ]; do
  case "$1" in
    --auth|--api-key-file)
...
for host in $hosts; do configure_host "$host" check; done
if [ "$check" = true ]; then
...
for host in $hosts; do configure_host "$host" install; done
