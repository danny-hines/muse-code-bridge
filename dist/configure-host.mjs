...
// scripts/configure-host.mjs
var serverName = "muse_code_bridge";
...
function jsonDocument(text) {
  const errors = [];
  const tree = parseTree2(text || "{}", errors, { allowTrailingComma: true });
...
function transformConfig({ host, text, command, muse, opencodeMajor = 1 }) {
  if (host === "hermes") {
    const doc = (0, import_yaml.parseDocument)(text, { version: "1.1", prettyErrors: false });
    if (doc.errors.length || doc.warnings.length || doc.contents !== null && !(0, import_yaml.isMap)(doc.contents)) throw new Error("Invalid or unsupported YAML configuration; no settings changed.");
    const data2 = doc.toJS() || {};
    if (data2.mcp_servers !== void 0 && !doc.has("mcp_servers")) throw new Error("An inherited mcp_servers map needs manual configuration.");
    if (data2.mcp_servers !== void 0 && !object(data2.mcp_servers)) throw new Error("mcp_servers must be a mapping.");
    const servers = doc.get("mcp_servers", true);
    if (servers && (!(0, import_yaml.isMap)(servers) || servers.anchor || servers.items.some((p) => p.key?.source === "<<"))) throw new Error("An aliased or merged mcp_servers map needs manual configuration.");
    const desired2 = { command: command[0], args: command.slice(1), env: { MUSE_BRIDGE_EXECUTABLE: muse }, timeout: 60 };
    if (checkExisting(data2.mcp_servers?.[serverName], desired2)) return text;
    if (doc.contents === null) doc.contents = doc.createNode({});
    if (!doc.has("mcp_servers")) doc.set("mcp_servers", doc.createNode({}));
    doc.setIn(["mcp_servers", serverName], desired2);
  ...
  if (data.mcp !== void 0 && !object(data.mcp)) throw new Error("mcp must be an object.");
  const v2 = opencodeMajor === 2;
  if (!v2 && data.mcp?.servers !== void 0) throw new Error("Configuration uses the OpenCode 2 layout; select --opencode-version 2.");
  ...
  if (v2 && data.mcp?.servers !== void 0 && !object(data.mcp.servers)) throw new Error("mcp.servers must be an object.");
...
async function configPath(host, env = process.env) {
...
async function configure({ host, repo, node, muse, check = false, opencodeVersion = "auto", env = process.env }) {
  ...
  if (host === "opencode" && opencodeVersion === "auto") {
    try {
      const v = execFileSync(env.MUSE_BRIDGE_OPENCODE_BIN || "opencode", ["--version"], { encoding: "utf8", timeout: 1e4, stdio: ["ignore", "pipe", "pipe"] });
  ...
  if (check) {
    console.log(`Checked ${host}: ${file}. No settings changed.`);
    return { file, changed: false };
  }
  if (next === text) {
    console.log(`${host} is already configured: ${file}`);
    return { file, changed: false };
  }
  ...
  try {
    ...
    if (entry) {
      ...
      await writeFile(backup, text, { flag: "wx", mode: 384 });
    }
    await writeFile(temp, next, { flag: "wx", mode: 384 });
...
var entryFile = process.argv[1] && await realpath(process.argv[1]).catch(() => null);
if (entryFile && import.meta.url === pathToFileURL(entryFile).href) {
  try {
    const { values } = parseArgs({ options: {
      ...
      "opencode-version": { type: "string", default: "auto" }
    } });
