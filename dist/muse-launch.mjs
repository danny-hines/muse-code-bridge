...
// scripts/launch-shared-macos.mjs
...
try {
  ...
  const version = execFileSync(config.codexBin, ["--version"], { encoding: "utf8", timeout: 1e4 }).trim();
  ...
  if (process.argv.includes("--check")) {
    ...
    const env = {
      ...process.env,
      ...config.environment,
      MUSE_BRIDGE_ROOT: config.bridgeRoot,
      MUSE_BRIDGE_EXECUTABLE: config.museBin,
      MUSE_SHARED_CODEX_BIN: config.codexBin,
      MUSE_SHARED_NODE_BIN: config.nodeBin,
      MUSE_SHARED_ENTRYPOINT: config.entrypoint,
      CODEX_CLI_PATH: shim,
  ...
  if (process.argv.includes("--check")) {
    ...
    try {
      execFileSync("/usr/bin/osascript", ["-e", script, message], { stdio: "ignore" });
    } catch {
    }
    process.exitCode = 1;
  }
