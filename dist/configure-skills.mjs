...
var manifestName = ".muse-code-bridge-skills.json";
...
function selectSkills(value = "all") {
...
function skillsPath(host, env = process.env) {
...
var hashes = (files) => files && Object.fromEntries(Object.entries(files).map(([key, data]) => [key, digest(data)]));
async function readManifest(root) {
  ...
  if (data?.version !== 1 || !Array.isArray(data.selected) || data.selected.some((n) => !names.includes(n)) || !data.files || typeof data.files !== "object" || Array.isArray(data.files) || !isDeepStrictEqual(Object.keys(data.files).sort(), [...new Set(data.selected)].sort()) || data.selected.length && !data.selected.includes("muse") || Object.values(data.files).some((files) => !files || typeof files !== "object" || Array.isArray(files) || Object.values(files).some((hash) => typeof hash !== "string" || !/^[a-f0-9]{64}$/.test(hash)))) {
    throw new Error("Invalid skill install manifest; no skills changed.");
  }
  return data;
}
async function plan({ root, repo, selection }) {
  ...
  const selected = selection === void 0 ? previous?.selected || names : selectSkills(selection);
  const desired = {};
...
async function configureSkills({ host, repo, selection, check = false, env = process.env }) {
  if (host === "codex") {
    if (selection !== void 0 && selection !== "all") throw new Error("Codex installs the complete skill collection with its plugin; --skills selection applies to Hermes/OpenCode.");
  ...
  const lock = await open(lockPath, "wx", 384);
  ...
  try {
    ...
    for (const change of changes) {
      ...
      for (const [file, data] of Object.entries(change.files)) {
        ...
        await mkdir(join(path, ".."), { recursive: true, mode: 448 });
