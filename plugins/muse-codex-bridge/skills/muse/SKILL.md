...
When troubleshooting an update, report `bridge_version`, `bridge_build`, and `bridge_started_at` from `muse_status`. These describe the running process; reading a newer skill or manifest does not reload it. Older versions omit these diagnostics. After an update, current work should finish before the user fully quits Codex (Cmd+Q on macOS) and reopens it. Do not kill unrelated processes to force a reload. An old bridge may report `failed/incomplete` prematurely while Muse continues; keep the session ID and check the saved session after restarting before considering a repeat submission.
